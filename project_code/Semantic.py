...
class Semantic_Analyser():
    ...
    def tool_print_semanticstack(self):
        print("------- 打印语义分析栈 -----------")
        for s in self.sStack:
            print(s.name)
        return

    def tool_print_functable(self):
        print("------- 打印函数表 -----------")
        for f in self.funcTable:
            paraList = []
    ...
    def tool_print_symboltable(self):
        print("------- 打印符号表 -----------")
        for s in self.symbolTable:
            print(s.name, s.type, s.place, s.function)
        return

    def tool_print_nodecode(self, node):
        print('------ 打印语法树节点的中间代码 -----------' + node.name)
        for code in node.code:
            print(code)
        return

    def tool_print_nodestack(self, node):
