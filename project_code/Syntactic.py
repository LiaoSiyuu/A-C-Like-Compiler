...
class Syntactic_Analyzer():
    ...
    def __init__(self, lex, cfg, family):
        ...
        self.semantic = Semantic_Analyser() # 语法分析的同时进行语义分析
    ...
    def tool_print_MTable(self):
        print('----------- 打印 M 表 --------------')
        for i in self.M.keys():
            print(i, self.M[i])
        return

    def tool_print_Tables(self):
        print('-------------- 打印ACTION表 --------------')
    ...
    def tool_print_steps(self, stateStack, shiftStr, inputStr):
        print('----------- 打印规约过程 --------------')
        shiftList = []
        inputList = []
        for s in shiftStr:
