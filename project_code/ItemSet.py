...
class ItemSetSpecificationFamily():
    ...
    # @ 函数名：GO
    # @ 函数功能：计算状态转换函数GO，GO(I，X) ＝ CLOSURE(J)
    def GO(self, I, X):
        J = []
            ...
            for I in iS:
                for X in self.symbols:
                    ...
                    if tempItemSet.toString() in setStrings.values():
                        tempItemSet.name = list(setStrings.keys())[
                            list(setStrings.values()).index(tempItemSet.toString())]
                    else:
...
class ItemSet:
    ...
    def __init__(self, name, items):
        ...
        for item in self.items:
            ...
            if item_str not in self.string:
                self.string.append(item_str)
...
class CFG:
    ...
    # 读取语法表达式文件
    def tool_read_GrammarFile(self, path):
        # 拓展成广义语法
        # 更改语法时需要修改此函数
        ...
        for t in tokens:
            ...
            for rightIdx in range(len(t['right'])):
                ...
                for rightIdx2 in range(len(t['right'][rightIdx])):  # 添加非变元
                    if t['right'][rightIdx][rightIdx2]['class'] == 'T' and t['right'][rightIdx][rightIdx2][
                        'type'] not in self.TerminalSymbols:
